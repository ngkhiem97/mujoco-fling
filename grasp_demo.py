...
class SimulatorVelCtrl: #a communication wrapper for MuJoCo
    ...
    def fling(self, gqtgt):
        time.sleep(3)

        # move down for 2 seconds
        self.twist_ee = np.array([0, 0, -0.99, 0, 0, 0])
        t0 = time.time()
        duration = 1.8
        ...
        axang = quat2axang(ang_v)
        tmp = axang[3]*axang[0:3]
        self.twist_ee = np.array([0.99, 0, 0.88,tmp[0]*3,tmp[1]*3,tmp[2]*3])
        t0 = time.time()
        duration = 2
        while time.time() - t0 < duration:
            self.lock.acquire()
            vtgt = self.velocityCtrl.get_joint_vel_worldframe(self.twist_ee, np.array(self.sim.data.qpos[0:7]), np.array(self.sim.data.qvel[0:7]))   
            self.queue.append(vtgt*14*3)                    
            self.lock.release()
            time.sleep(0.1)

        time.sleep(0.1)
        
        self.twist_ee = np.array([-0.99, 0, -0.88*0.8, -tmp[0]*3*0.8, -tmp[1]*3*0.8, -tmp[2]*3*0.8])
        t0 = time.time()
        duration = 5
        while time.time() - t0 < duration:
            self.lock.acquire()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action', type=str, default='normal')
    args = parser.parse_args()
    action = args.action
    
    simulator = SimulatorVelCtrl()
    simulator.init("gen3_robotiq_2f_85_cellphone_table_grasp_multi_cloth.xml", 7, action)
    simulator.start()